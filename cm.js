...
export default class NestedCases extends NavigationMixin(LightningElement) {
    ...
    listviewOptions = [
        ...
        { label: 'R1', value: 'DUNNING_R1' },
        { label: 'R2', value: 'DUNNING_R2' },
        { label: 'R3', value: 'DUNNING_R3' },
        { label: 'LFO', value: 'DUNNING_LFO' },
        { label: 'Termination', value: 'DUNNING_TERMINATION' }
    ];
    ...
    console.log('Length:', this.visibleRows.length);
...
handleInterestResizeMouseUp() {
    ...
    buildRows() {
        ...
        this.visibleRows = this.allData.map((row, index) => ({
            ...
            DunningDate: row.DunningDate || null,
           ...
           // BilledPercentAgainstTSV: row.BilledPercentAgainstTSV != null ? row.BilledPercentAgainstTSV : '',
           FormattedBilledAgainstTSV:
    this.formatRs(row.TotalBilledAmount) + ' (' + (row.BilledPercentAgainstTSV || 0) + '%)',
FormattedReceivedAgainstTSV:
    this.formatRs(row.TotalAmountReceived) + ' (' + (row.BillingReceivedPercentAgainstTSV || 0) + '%)',
    FormattedOutstandingAgainstTSV:
    this.formatRs(row.OutstandingAmountAgainstTSV) + ' (' + (row.OutstandingPercentAgainstTSV || 0) + '%)',
            ...
            EPDPartialPayment:
    row.EPDPartialPayment != null
        ? row.EPDPartialPayment
        : 0,
           ...
           FormattedBalanceInterest  : (row.BalanceInterest != null ? Number(row.BalanceInterest) : 0)
    .toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 }),
FormattedTotalHoldingCharge: (row.TotalHoldingCharge != null ? Number(row.TotalHoldingCharge) : 0)
    .toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 }),
    FormattedTSVAllInclusive: (row.TSVAllInclusive != null ? Number(row.TSVAllInclusive) : 0)
    .toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 }),
...
selectActivityOption(event) {
    ...
    // ↓ ADD THIS BLOCK ↓
        ...
        const isUnbilledOrIneligible = isCurrentMonth;
...
const discountOptionsWithState = DISCOUNT_OPTIONS.map(opt => ({
    ...opt,
...
if (d.BillingDueDate) {
    ...
    console.log(
        ...
        //return {
           ...
           TDS1Percent: d.TdsAmount != null ? d.TdsAmount : '',
            ...
            disableDiscountApplied: isUnbilledOrIneligible,
            disablePartialPayment: isUnbilledOrIneligible,
            disableProjectedDate: isUnbilledOrIneligible,
        ...
        return this.recomputeDemandRow(milestoneRow);
    }
    ...
    recomputeDemandRow(demand) {
        if (demand.BillingDueDate && demand.ProjectedPaymentDate) {
            ...
            const today = new Date();
            const todayOnly = new Date(today.getFullYear(), today.getMonth(), today.getDate());
            const diffTime = dueDate.getTime() - todayOnly.getTime();
            demand.EarlierDays = Math.floor(diffTime / (1000 * 60 * 60 * 24));
...
/*
        if (demand.DiscountApplied === 'Yes') {
            const daysEarly = Number(demand.EarlierDays) || 0;
            demand.DiscountCalculatedValue =
                (demand.TotalBilledLessTDS != null && daysEarly > 0)
                    ? (Number(demand.TotalBilledLessTDS) * DISCOUNT_PERCENT * (daysEarly / 365)).toFixed(2)
                    : '0.00';
        } else {
            demand.DiscountCalculatedValue = '';
        }
        ...
        */
        ...
        if (demand.DiscountApplied === 'Applicable') {
        const daysEarly = Number(demand.EarlierDays) || 0;
        const partialAmt = Number(demand.PartialPaymentAmount) || 0;
        demand.DiscountCalculatedValue =
        (partialAmt > 0 && daysEarly > 0)
            ? (partialAmt * DISCOUNT_PERCENT * (daysEarly / 365)).toFixed(2)
            : '0.00';
} else {
    demand.DiscountCalculatedValue = '';
}

const epdPartialAmt = Number(demand.PartialPaymentAmount) || 0;
const epdDiscount = Number(demand.DiscountCalculatedValue) || 0;
demand.EPDPartialPayment = (epdPartialAmt - epdDiscount).toFixed(2);
...
const isOverdue =
    ...
    demand.disableDiscountApplied = isOverdue;
    demand.disablePartialPayment = isOverdue;
    demand.disableProjectedDate = isOverdue;
    ...
    this.visibleRows = this.visibleRows.map(bookingRow => ({
        ...
        demandRows: bookingRow.demandRows
            ? bookingRow.demandRows.map(d => {
                ...
                const billingDueDate = d.BillingDueDate ? new Date(d.BillingDueDate) : null;
const today = new Date();
const isCurrentMonth = billingDueDate &&
    billingDueDate.getFullYear() === today.getFullYear() &&
    billingDueDate.getMonth() === today.getMonth();
const isUnbilledOrIneligible =  isOverdue;
                return {
                    ...d,
...
// Validate PTP Date (cannot be in the past)
const today = new Date();
...
for (const row of this.visibleRows) {
    ...
    if (row.PTP_Date) {
        ...
        const ptpDate = new Date(row.PTP_Date);
        ...
        if (ptpDate < today) {
            ...
            this.dispatchEvent(
                new ShowToastEvent({
                    title: 'Validation Error',
                    message: `PTP Date cannot be in the past for Booking ${row.BookingName}. Please select today or a future date.`,
                    variant: 'error'
                })
            );
